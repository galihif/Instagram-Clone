...
class LoginViewModelTest{
    ...
    @Test
    fun `when login should return true and not null`() = mainCoroutineRules.runBlockingTest {
        val dummyResult = MutableLiveData<Boolean>(true)
        val dummyUser = DataDummy.generateDummyUser()
        Mockito.`when`(viewModel.login(dummyUser)).thenReturn(dummyResult)

        val actualResult = viewModel.login(dummyUser).getOrAwaitValue()
        Mockito.verify(viewModel).login(dummyUser)
        Assert.assertNotNull(actualResult)
        Assert.assertEquals(dummyResult.value, actualResult)
    }

    @Test
    fun `when login should return false and not null`() = mainCoroutineRules.runBlockingTest {
